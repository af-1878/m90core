...
always_ff @(posedge clk) begin
    ...
    if (reset) begin
        // Start just past VBlank boundary to give V35 maximum time (~15ms)
        // to initialise its interrupt vectors before first VBlank fires
        hcnt <= 10'd101;  // start of active horizontal
        vcnt <= 10'd137;  // just past VBlank (vblank=0 when vcnt >= 136)
